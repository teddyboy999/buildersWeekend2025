...
@morph.func 
@morph.load_data("testing_data_iowa_housing_prices")
def testing_ShowData(context: MorphGlobalContext):
    df = context.data["testing_data_iowa_housing_prices"]

    # Plot Bar Graph as a Figure to visually represent Data 
    fig = px.bar(df, x="SquareFeet", y="SalePrice")
    return fig
