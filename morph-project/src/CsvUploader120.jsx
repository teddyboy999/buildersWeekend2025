...
const CsvUploader120 = ({ onDataLoaded }) => {
  ...
  const handleFileUpload = async (event) => {
    ...
    Papa.parse(file, {
      ...
      complete: async (result) => {
        const expectedColumns = ['product_id', ...Array.from({ length: 120 }, (_, i) => `day${i + 1}`)];
        if (!expectedColumns.every(col => col in result.data[0])) {
          setStatus('CSV must have product_id, day1, day2, ..., day120');
          return;
        }
        ...
        const formattedData = result.data.map(row => ({
          ...
          ...Object.fromEntries(
            Array.from({ length: 120 }, (_, i) => [`day${i + 1}`, parseInt(row[`day${i + 1}`]) || 0])
          ),
        }));
        ...
        // Update the spinlock so XpressAI can check
        try 
        {
          const { error } = await supabase
            .from('spinlock')
          ...
          if (error) throw error;
          setStatus('SpinLock updated in Supabase');
        } 
        catch (error) 
        {
          setStatus(`Error uploading: ${error.message}`);
